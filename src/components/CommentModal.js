  ...
  const handlePostComment = async () => {
    ...
    try {
      ...
      await addDoc(collection(db, 'posts', postId, 'comments'), {
        ...
        user: { username: userData.username, avatar: userData.avatar || 'https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_1280.png' },
      ...
      <View style={[styles.modalContent, { paddingBottom: insets.bottom || 20 }]}>
        ...
        {isLoading ? (
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <View style={styles.commentRow}>
                ...
                <View style={styles.commentContent}>
                  <Text style={styles.commentText}><Text style={styles.username}>{item.user?.username} </Text>{item.text}</Text>
                </View>
              </View>
            )}
            ...
            ListEmptyComponent={<Text style={styles.emptyText}>No comments yet. Start the conversation!</Text>}
          />
        )}
        ...
        <View style={styles.inputContainer}>
          ...
          <TouchableOpacity onPress={handlePostComment} disabled={isPosting || !newComment.trim()}>
            {isPosting ? <ActivityIndicator size="small" color="#007AFF" style={{ marginBottom: 10 }} /> : <Text style={[styles.postButtonText, { color: newComment.trim() ? '#007AFF' : '#A0A0A0' }]}>Post</Text>}
          </TouchableOpacity>
        </View>
      </View>
    </Modal>
